...
describe('Visits in PoetrySlamManager Service', () => {
  ...
  it('should change the status of visit in action cancel on booked entity', async () => {
    ...
    const actionResult = await ACTION(
      `/odata/v4/poetryslammanager/PoetrySlams(ID=${visit.parent_ID},IsActiveEntity=true)/visits(ID=${visit.ID},IsActiveEntity=true)`,
      'cancelVisit'
    );
    ...
    let result = await GET(
      `/odata/v4/poetryslammanager/PoetrySlams(ID=${visit.parent_ID},IsActiveEntity=true)/visits(ID=${visit.ID},IsActiveEntity=true)?$expand=parent`,
      {
