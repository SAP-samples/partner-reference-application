...
describe('Remote Projects in PoetrySlamManager Service', () => {
  ...
  it('should throw error message when retrieving SAP HANA Cloud project and no system is connected', async () => {
    expect(
      GET(
        `/odata/v4/poetryslammanager/S4HCProjects(projectUUID=00000000-0000-0000-0000-000000000000)`
      )
    ).to.rejectedWith(500);
  });
  ...
  it('should throw error message when retrieving SAP HANA Cloud processing status and no system is connected', async () => {
    expect(
      GET(
        `/odata/v4/poetryslammanager/S4HCProjectsProcessingStatus(processingStatus='aa')`
      )
    ).to.rejectedWith(500);
  });
  ...
  it('should throw error message when retrieving SAP HANA Cloud project profile code and no system is connected', async () => {
    expect(
      GET(
        `/odata/v4/poetryslammanager/S4HCProjectsProjectProfileCode(projectProfileCode='aaaaaaa')`
      )
    ).to.rejectedWith(500);
  });
  ...
  it('should throw error message when retrieving SAP Business ByDesign project and no system is connected', async () => {
    expect(
      GET(
        `/odata/v4/poetryslammanager/ByDProjects(ID='00000000-0000-0000-0000-000000000000')`
      )
    ).to.rejectedWith(500);
  });
  ...
  it('should throw error message when retrieving SAP Business One purchase order and no system is connected', async () => {
    expect(
      GET(`/odata/v4/poetryslammanager/B1PurchaseOrder(docEntry=1)`)
    ).to.rejectedWith(500);
  });
});
